...
export interface TherapyLessonInput {
  levelId: string;
  levelLabel: string;
  lessonNumber: number;
  title: string;
...
function autoVocabMCQs(vocab: TherapyLessonInput["vocab"]): MCQItem[] {
...
function autoConversationMCQs(input: TherapyLessonInput): MCQItem[] {
  return input.concept.tips.slice(0, 5).map((tip) => ({
    question: `في السياق العلاجي: ${tip.sentence}`,
    options: [
      tip.note,
      "لا يوجد أثر تعليمي.",
      "يُستخدم فقط مع الكبار.",
      "خطوة اختيارية غير مهمة.",
    ],
    correct: 0,
  }));
}

function autoGrammarMCQs(input: TherapyLessonInput): MCQItem[] {
...
export function buildTherapyLesson(input: TherapyLessonInput): LessonData {
  ...
  return {
    ...
    grammarExercises: autoGrammarMCQs(input),
